...
type NdArrayDevice = burn::backend::ndarray::NdArrayDevice;
type WgpuDevice = burn::backend::wgpu::WgpuDevice;
...
impl PsychedelicVisualizer {
    ...
    fn run(&mut self) {
        ...
        println!("Controls: S/M/N/E=Operations, G=GPU/CPU, ↑↓=Speed, ←→=Intensity, Space=Status, Esc=Exit");
    ...
    fn handle_input(&mut self) {
        ...
        if self.window.is_key_pressed(Key::Space, minifb::KeyRepeat::No) {
            println!("Status: Op={}, Backend={}, Speed={:.2}, Intensity={:.2}", 
