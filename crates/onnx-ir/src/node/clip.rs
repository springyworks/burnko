...
pub fn clip_config(node: &Node) -> (Option<f64>, Option<f64>) {
    ...
    if min_result.is_none() && max_result.is_none() {
        ...
        if min_result.is_none() && let Some(min_value) = min {
            let min = min_value.data.into_scalar();
        ...
        if max_result.is_none() && let Some(max_value) = max {
            let max = max_value.data.into_scalar();
