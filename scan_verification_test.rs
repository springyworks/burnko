...
fn main() {
    ...
    // Test 4: Large scale cumsum (simulated 50M elements with smaller but representative test)
    ...
    let large_tensor = Tensor::<TestBackend, 1>::from_data(
        TensorData::new(large_data, Shape::new([large_size])).convert(),
        &device
    );
    ...
    // Test 6: Random data robustness
    println!("\n6. Testing with random data...");
    let random_tensor = Tensor::<TestBackend, 1>::random([1000], Distribution::Default, &device);
    let random_result = random_tensor.cumsum(0);
    println!("Random tensor shape: {:?}", random_tensor.shape());
    println!("Random result shape: {:?}", random_result.shape());
    assert_eq!(random_result.shape(), random_tensor.shape());
    ...
    println!("• Random data robustness");
}
